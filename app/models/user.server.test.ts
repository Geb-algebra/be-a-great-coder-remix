...
describe('createUser', async () => {
  it('creates a right record', async () => {
    ...
    await createUser('Alice', 'Alices_password');
    ...
    const hashedPassword = await bcrypt.hash('Alices_password', 10);
    expect(bcrypt.compareSync('Alices_password', hashedPassword)).toBeTruthy();
  });
});
...
describe('verifyLogin', async () => {
  const name = 'Alice';
  const password = 'Alices_password';
  beforeEach(async () => {
  ...
  it('should return user info except the password', async () => {
    ...
    expect(new Set(Object.keys(userInfo!))).toEqual(
      new Set(['id', 'name', 'createdAt', 'updatedAt']),
    );
  });
  it('should return null if the password is wrong', async () => {
    const userInfo = await verifyLogin(name, 'alice_spassword');
